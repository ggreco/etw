...
void SetShotSpeed(Giocatore *g, WORD distance)
{
...
void SetComando(Giocatore *g,BYTE cmd,BYTE dopo,BYTE arg)
{
	switch(cmd)
	{
		case ESEGUI_ROTAZIONE:
			{
				BYTE NewDir=(g->Direzione-arg);
...
Si tira anche girati di schiena!
				...
				if(	temp==g->Direzione ||
					temp==dir_next[g->Direzione] ||
					temp==dir_pred[g->Direzione] )
...
Giocatore *TrovaPiuVicinoDirGiocatore(Giocatore *g)
{
        register Giocatore *g2,*g_min=NULL;
        WORD d_min=31000,t;
        register int i;
...
Giocatore *TrovaPiuVicinoGiocatore(Giocatore *g)
{
        register Giocatore *g2,*g_min=NULL;
        WORD d_min=31000,t;
        register int i;
...
void MoveTo(Giocatore *g,WORD xd,WORD yd)
{
	...
	{
		if(NewDir!=g->Direzione)
		{
			if(NewDir==opposto[g->Direzione])
			{
				if(g==pl->gioc_palla)
				{
...
// = pl->velocita*2
	...
	pl->delta_y=-(cos_table[pl->Direzione]*pl->velocita)>>6;
	pl->delta_x=(sin_table[pl->Direzione]*pl->velocita)>>6;
}
...
BOOL AllowRotation(Giocatore *g,WORD NewDir)
{
	register WORD result=abs(g->Direzione-NewDir);
...
// Modifica, ora lo faccio SOLO se la palla NON e' libera (quindi e' dell'altra squadra)...
                ...
                g->world_y>(pl->world_y+530 ) )
	{
		...
		if(i>=0 && (r_controls[i][counter]&JP_DIRECTION_MASK) )
		{
			...
			g2=TrovaPiuVicino(s,G2P_X(pl->world_x),G2P_Y(pl->world_y));
...
void EseguiDopoComando(Giocatore *g)
{
	switch(g->CA[0])
	{
		case ESEGUI_TIRO:
		...
		case ESEGUI_RIMESSA:
			...
			break;
		case ESEGUI_BATTUTA:
			g->Direzione=FindDirection(g->world_x,g->world_y,g->SNum ? PORTA_E_X : PORTA_O_X,PORTA_O_Y);
...
WORD IndirizzaTiro(Giocatore *g,ULONG joystate)
{
	...
	pl->Direzione=FindDirection256(g->world_x,g->world_y,dest_x,dest_y);

	return (WORD)FindDistance(g->world_x,g->world_y,dest_x,dest_y,pl->Direzione);
}
