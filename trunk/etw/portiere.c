...
void CheckPortiere(int n)
{
	...
	if(pl->TipoTiro==TIRO_ALTO || pl->TipoTiro==TIRO_PALLONETTO)
	{
		...
		if(g->SpecialData!=-1)
		{
			g->Direzione=opposto[g->SpecialData];
...
void HandleGoalKick(struct Portiere *g)
{
...
rinvio:
			...
			pl->Direzione=(olddir<<5)+(GetTable()<<1)-5;

			if(g->Direzione==D_SUD_EST ||
				g->Direzione==D_NORD_OVEST )
				pl->Direzione+=(GetTable()+2)<<1;
			else if(g->Direzione==D_NORD_EST ||
...
rinviocomputer:
			...
			FermaPalla();
			pl->Direzione=(g->Direzione<<5)-10+(GetTable()<<2);
                        g->TimePress=0;
			g->AnimFrame=3;
		}
	}
...
void HandlePortiereControlled(struct Portiere *g)
{
	if(p->show_panel&0xff00)
		return;
...
void HandlePortiere(int num)
{
	...
	if(g->Tick<g->Attenzione&&!g->Special)
	{
	    ...
	    {
		g->Direzione=FindDirection(g->world_x,g->world_y,pl->world_x-48,pl->world_y-100);
		g->ActualSpeed=0;
	...
	if(g->ActualSpeed>0)
	{
		g->world_x+=velocita_x[g->ActualSpeed][g->velocita][g->Direzione];
		g->world_y+=velocita_y[g->ActualSpeed][g->velocita][g->Direzione];
	}
	else if(g->Special)
	{
	...
	/*		g->team!=pl->sq_palla    && */
	                pl->world_y<(g->world_y+200) )
		{
			...
			if(pl->gioc_palla)
			{
				if((g->Parata+GetTable()+2)>pl->gioc_palla->Tecnica)
				{
...
// Limiti per non far tuffare il portiere dentro la rete.
			...
			{
				BYTE temp=CambioDirezione[g->Direzione][g->SpecialData>>5];
...
// Tuffo sui piedi, da fare...
			...
			else if(pl->gioc_palla)
			{
				...
				if((g->Parata+GetTable()+3)>pl->gioc_palla->Tecnica)
				{
					DoSpecialAnim(pl->gioc_palla,GIOCATORE_CADUTA);
...
skiptuffo:
	...
	if(g->FrameLen<=0)
	{
		...
		if(g->OnScreen)
			ChangeAnimObj(g->immagine,PortAnim[g->AnimType].Frame[g->AnimFrame]+g->Direzione);
	}
