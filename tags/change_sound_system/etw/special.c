...
void FreePass(Giocatore *g,ULONG joystate)
{
	pl->velocita=23;

	pl->Direzione=(g->Direzione<<5);

	if(	joystate&mov_dec_angolo[g->Direzione][0] &&
		!(joystate&mov_dec_angolo[g->Direzione][1])
		)
	{
		pl->Direzione-=(10+g->Creativita-GetTable());
	}
	else if(joystate&mov_inc_angolo[g->Direzione][0] &&
		!(joystate&mov_inc_angolo[g->Direzione][1])
		)
	{
		pl->Direzione+=(10+g->Creativita-GetTable());
	}

	if( joystate&joy_opposto[g->Direzione] &&
		!(joystate&joy_opposto_neg[g->Direzione]) 
	)
	{
// Pallonetto
		pl->TipoTiro=TIRO_PALLONETTO;
	}
	else if( joystate&joy_opposto[opposto[g->Direzione]] &&
		!(joystate&joy_opposto_neg[opposto[g->Direzione]] )
	)
	{
// Tiro rasoterra
		pl->velocita>>=1;
...
void TargetedPass(Giocatore *g,ULONG joystate)
{
	...
	g2=TrovaPiuVicinoGiocatore(g);
	pl->Direzione=FindDirection256(old_x,old_y,g2->world_x,g2->world_y);
	d=FindDistance(old_x,old_y,g2->world_x,g2->world_y,pl->Direzione);
	...
	if(d<300)
	{
		...
		if(g2->ActualSpeed)
		{
			pl->Direzione=FindDirection256(old_x,old_y,
				g2->world_x+(velocita_x[g->ActualSpeed-1][g->Velocita][g->Direzione]<<3),
				g2->world_y+(velocita_y[g->ActualSpeed-1][g->Velocita][g->Direzione]<<3));
		}
		else
		{
			if(g2->SNum)
			{
				if(pl->Direzione>196||pl->Direzione<64)
					pl->Direzione+=10;
				else
					pl->Direzione-=10;
			}
...
// L'omino ideale e' 20 pixel prima del dischetto del rigore
	...
	pl->Direzione=FindDirection256(old_x,old_y,g2->world_x,g2->world_y);
	distance=FindDistance(old_x,old_y,g2->world_x,g2->world_y,pl->Direzione);
...
// Do un bonus qui, senno' mi vengono troppo corti i cross.
	...
	pl->MaxQuota=9+GetTable()+g->Creativita;
...
// Se e' sotto metto un omino sotto e tre sopra, altrimenti tre sotto e uno sopra.
	...
	for(i=0;i<4;i++)
	{
		g=TrovaPiuVicino(s,get_position,CENTROCAMPO_Y);
...
// 80 era velocita_palla[20]...
	...
	pl->delta_y=-(cos_table[pl->Direzione]*(80))>>7;
	pl->delta_x=(sin_table[pl->Direzione]*(80))>>7;
...
// Non usava G2P ma un +128 su g->world_x
        	        ...
        	        {
                	        register Giocatore *g2=pl->gioc_palla;

	                        if(g2->ActualSpeed>0)
        	                {
                	                g->SpecialData=FindDirection32(g->world_x,g->world_y+128,
                        	                pl->world_x+(velocita_x[g2->ActualSpeed-1][g2->Velocita][g2->Direzione]<<3),
                                	        pl->world_y+(velocita_y[g2->ActualSpeed-1][g2->Velocita][g2->Direzione]<<3) );
	                        }
        	                else
                	                g->SpecialData=FindDirection32(g->world_x,g->world_y+128,pl->world_x,pl->world_y);

	                }
	                g->Direzione=g->SpecialData>>2;
...
// Adesso si fanno in 12 passi, non piu' in 6
			...
			g->Direzione=g->SpecialData>>5;
			g->ActualSpeed=0;

			if(pl->quota<8)
			{
				if(CambioDirezione[g->Direzione][PortaDir]<2 &&
					CambioDirezione[g->Direzione][PortaDir]>-2)
				{
			...
			{
				if(CambioDirezione[g->Direzione][PortaDir]!=4)
				{
...
void PreparaPassaggio(Giocatore *g,Giocatore *g_min)
{
	...
	if(g_min->ActualSpeed>0)
	{
	    ...
	    {
		ExpectedX+=(velocita_x[g_min->ActualSpeed-1][g_min->Velocita][g_min->Direzione]<<5);
		ExpectedY+=(velocita_y[g_min->ActualSpeed-1][g_min->Velocita][g_min->Direzione]<<5);
	    }
	    else
	    {
		ExpectedX+=(velocita_x[g_min->ActualSpeed-1][g_min->Velocita][g_min->Direzione]<<2);
		ExpectedY+=(velocita_y[g_min->ActualSpeed-1][g_min->Velocita][g_min->Direzione]<<2);
	    }
	}
        pl->TipoTiro=TIRO_RASOTERRA;

        pl->Direzione=FindDirection256(g->world_x,g->world_y,ExpectedX,ExpectedY);
	distance=FindDistance(g->world_x,g->world_y,ExpectedX,ExpectedY,pl->Direzione);
...
void Passaggio2(Giocatore *g,char Dir)
{
	WORD olddir=g->Direzione;

	g->Direzione=Dir;

	Passaggio(g);

	if(g->Direzione!=olddir)
	{
		BYTE NewDir=g->Direzione;
...
void RimessaLaterale(Giocatore *g)
{
...
// Serve per evitare che durante la rimessa venga mostrato il pannello
	...
	p->arbitro.Comando=METTI_IN_GIOCO;
	p->arbitro.Tick=8;
...
void HandleRimessa(Giocatore *g)
{
	...
	if(g->squadra->Joystick>=0)
	{
		...
		if(!(l&JP_DIRECTION_MASK))
		{
			if(g->world_y<CENTROCAMPO_Y)
				g->Direzione=D_SUD;
			else g->Direzione=D_NORD;

			return;
		}

		g->Direzione=GetJoyDirection(l);
		...
		if(g->world_y<CENTROCAMPO_Y)
		{
			switch(g->Direzione)
			{
				case D_NORD:
					...
					break;
				case D_NORD_EST:
					g->Direzione=D_SUD_EST;
					break;
				case D_NORD_OVEST:
					g->Direzione=D_SUD_OVEST;
					break;
			}
		}
		else
		{
			switch(g->Direzione)
			{
				case D_SUD:
					...
					break;
				case D_SUD_EST:
					g->Direzione=D_NORD_EST;
					break;
				case D_SUD_OVEST:
					g->Direzione=D_NORD_OVEST;
					break;
			}
		}
	}
...
rimessacomputer:
			...
			Passaggio(g);
			g->TimePress=-g->SpecialData;
			g->SpecialData=4+g->Creativita+GetTable();
			DoSpecialAnim(g,GIOCATORE_RIMESSA);
		}
...
/*
	Qui non dovrebbe piu' servire
		...
		g->world_x=pl->world_x-avanzamento_x[g->Direzione];
...
//			pl->Direzione=FindDirection256(pl->world_x+24,pl->world_y+24,(p->marker_x+9)<<3,(p->marker_y+4)<<3);

finetiro:
			pl->Direzione=FindDirection256(pl->world_x+24,
				pl->world_y+24,((p->marker_x+9)<<3)+(g->SNum ? (200) : (-200) ) ,
				(p->marker_y+4)<<3);

			D(bug("Direzione rigore: %ld (wx: %ld, mx: %ld)\n",pl->Direzione,pl->world_x,p->marker_x));
...
void PunizioneCorner(Giocatore *g)
{
	if(p->show_panel&0xff00)
		return;
...
*/
				...
				else
				{
					...
					pl->Direzione=FindDirection256(g->world_x,g->world_y,dest_x+((GetTable()-3)<<4),CENTROCAMPO_Y);

					pl->MaxQuota=GetTable()+g->Creativita+6;
...
void ColpoDiTesta(Giocatore *g)
{
	...
	if(CanScore(g)==CS_SI)
	{
		...
		temp=abs(CambioDirezione[pl->Direzione>>5][PortaDir]);
...
// passaggio
		...
		temp=CambioDirezione[olddir][g->Direzione];
		...
		if(temp>3)
		{
			...
			else if(g->AnimType==GIOCATORE_PRETUFFO)
			{
				...
				pl->MaxQuota=10+GetTable();
				pl->velocita=10;
				pl->Direzione=(g->Direzione<<5)+(GetTable()<<1)-5;
			}
			else 
			{
				pl->velocita=6;
