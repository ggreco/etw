...
{
	register int j,c0=p->squadra[0]->MarkerRed,c1=p->squadra[1]->MarkerRed;
	int main_bitmap_start=(int)main_bitmap+bitmap_width;
	int main_bitmap_end=(int)main_bitmap+bitmap_width*(bitmap_height-1);
	...
	if(small_dot)
	{
		...
		for(j=0;j<10;j++)
		{
			...
			if((int)bmt<main_bitmap_end&& (int)bmt>main_bitmap_start)
				*bmt=c0;
...
// Faccio 4 punti per l'omino attivo
	...
	if(small_dot)
	{
		...
		if((int)bm2<main_bitmap_end && (int)bm2>main_bitmap_start)
		{
			*bm2=c0;
			bm2+=bitmap_width;
			*bm2=c0;
	...
	if((int)bm2>main_bitmap_start && (int)bm2<main_bitmap_end)
	{
		if(small_dot)
		{
