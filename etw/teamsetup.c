...
void CheckPelle(void)
{
    ...
    /* AC: I put Progress() out of the for, so the number of progres blocks
     * are 20 and not 22, in order to have a plain result from the division
     * between window width and number of blocks. */
    Progress();
...
void DisponiPortiere(team_t *s, int sector, BOOL possesso)
{
    ...
    if(sector == GOALKICK)
    {
        if(possesso)
        {
            ...
            {
                s->keepers.world_x = (s == p->team[1] ? 70 : 1196) * 8;
                s->keepers.world_y = ((346-14)*8);
            }
            else
            {
                s->keepers.world_x = (s == p->team[1] ? 80 : 1186) * 8;
...
void DisponiSquadra(team_t *s, int sector, BOOL possesso)
{
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        if(s->players[i].AnimType != GIOCATORE_ESPULSO
            ...
            && s->players[i].Comando != ESCI_CAMPO)
        {
...
//    fread(&sd, sizeof(struct team_disk), 1, fh); prima la leggevo cosi'...
    ...
    if(sd.jerseys[0].color0 != COLOR_STANDARD_MAGLIE)
        RemapAnimObjColor(players[snum], Pens[COLOR_STANDARD_MAGLIE],
                                         Pens[sd.jerseys[0].color0]);
    if(sd.jerseys[0].color1 != COLOR_STANDARD_CALZONI)
        RemapAnimObjColor(players[snum], Pens[COLOR_STANDARD_CALZONI],
                                         Pens[sd.jerseys[0].color1]);
    if(sd.jerseys[0].color2 != COLOR_STANDARD_AUSILIARIO
         && sd.jerseys[0].type > 0)
        RemapAnimObjColor(players[snum], Pens[COLOR_STANDARD_AUSILIARIO],
                                         Pens[sd.jerseys[0].color2]);
...
// Copio i dati dei giocatori.
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        if(sd.players[i].Ammonizioni > 1)
        {
            ...
            while(k < sd.nplayers&&(sd.players[k].Ammonizioni > 1
                   || sd.players[k].injury > 2))
                k++;
        ...
        {
            uint8_t t = 2 * max(sd.players[i].injury, 3);
...
void LiberaPartita(game_t *game)
{
    ...
    if (arcade)
    {   // restore the original goalkeeper position
        int j;
...
void MakeResult(void)
{
    ...
    rectfill(p->result, 0, 0, result_width - 1, VS_CHAR_Y,
             Pens[P_VERDE2], result_width);
