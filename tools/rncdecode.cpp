...
#define RNC_SIGNATURE   0x524E4301
...
#define READ_BE_UINT16(x) ntohs(((uint16_t*)(x))[0])
#define READ_BE_UINT32(x) ntohl(((uint32_t*)(x))[0])

#if __BYTE_ORDER == __BIG_ENDIAN
#define READ_LE_UINT16(x) (uint16_t *)(x << 8 | x >> 8)
#else
#define READ_LE_UINT16(x) ((uint16_t *)(x))[0]
#endif
...
int main(int argc, char *argv[])
{
    ...
    if (FILE *in = fopen(argv[1], "rb")) {
        std::string src;
        void *dest = malloc(1024 * 1024);
        while (!feof(in)) {
            char buffer[1024];
            int len = fread(buffer, 1, sizeof(buffer), in);
            if (len > 0)
                src.append(buffer, len);
        }
        fclose(in);
        std::cerr << "Unpacking " << argv[1] << " (" << src.size() << " bytes)...";
        RncDecoder dec;
        int len = dec.unpackM1(src.c_str(), dest);

        if (len <= 0) 
            std::cerr << "ERROR " << len << '\n';
        else
            std::cerr << "OK (" << len << " bytes)\n";

        if (FILE *out = fopen(argv[2], "wb")) {
            fwrite(dest, 1, len, out);
            fclose(out);
        }
        else
            std::cerr << "Unable to open " << argv[2] << " for writing!\n";
